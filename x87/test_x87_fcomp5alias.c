...
int main() {
    ...
    for (size_t i = 0; i < sizeof(test_cases)/sizeof(test_cases[0]); ++i) {
        // Reset x87 state and mask all exceptions before each test
        asm volatile ("fninit\n\t"
                      "fldcw %0" 
                      : 
                      : "m" (cw));
        ...
        // Load values into x87 stack (need 6 levels to get ST(5))
        asm volatile (
            "fldt %1\n\t"    // load b into ST(0)
            "fldz\n\t"        // push 0 (b moves to ST(1))
            "fldz\n\t"        // push 0 (b moves to ST(2))
            "fldz\n\t"        // push 0 (b moves to ST(3))
            "fldz\n\t"        // push 0 (b moves to ST(4))
            "fldt %0\n\t"    // load a into ST(0), others move down
            "fcomp %%st(5)\n\t" // compare ST(0) with ST(5) and pop (FCOMP5alias)
            "fnop\n\t"        // for alignment
            : 
            : "m" (a), "m" (b)
