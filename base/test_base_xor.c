...
static void test_xor_combinations() {
    ...
    asm volatile (
        ...
        "pushfq\n\t"
        "popq %[flags_out]\n\t"
        : [result] "=a" (u8_result), [flags_out] "=r" (flags_after)
        : [mem_val] "m" (mem8)
        : "cc", "memory"
    );
    u8_expected = 0xFF;
    ...
    asm volatile (
        ...
        "pushfq\n\t"
        "pop %[flags_out]\n\t"
        : [result] "=a" (u16_result), [flags_out] "=r" (flags_after)
        : [mem_val] "m" (mem16)
        : "cc", "memory"
    );
    u16_expected = 0xFFFF;
    ...
    asm volatile (
        ...
        "pushfq\n\t"
        "pop %[flags_out]\n\t"
        : [result] "=a" (u32_result), [flags_out] "=r" (flags_after)
        :
        : "ebx", "cc", "memory"
    );
    ...
    asm volatile (
        ...
        "pushfq\n\t"
        "pop %[flags_out]\n\t"
        : [result] "=a" (u64_result), [flags_out] "=r" (flags_after)
        :
        : "rbx", "cc", "memory"
    );
