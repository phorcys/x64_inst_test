...
int main() {
    printf("SHLD instruction test\n");
    printf("=====================\n");
    
    int errors = 0;
    
    // Test 16-bit SHLD with immediate count
    ...
    printf("32-bit SHLD (<<16): 0x%08X\n", dest32);
    if (dest32 != expected32) {
        printf("Error: Expected 0x%08X, got 0x%08X\n", expected32, dest32);
        errors++;
    }

    // Test 64-bit SHLD with immediate count
    ...
    printf("64-bit SHLD (<<32): 0x%016lX\n", dest64);
    if (dest64 != expected64) {
        printf("Error: Expected 0x%016lX, got 0x%016lX\n", expected64, dest64);
        errors++;
    }

    // Test memory operand SHLD
    uint16_t mem_dest = 0x4000;
    uint16_t mem_src = 0x8000;
    ...
    printf("Memory SHLD (<<4): 0x%04X\n", mem_dest);
    if (mem_dest != expected_mem) {
        printf("Error: Expected 0x%04X, got 0x%04X\n", expected_mem, mem_dest);
        errors++;
    }

    // Test carry flag (CF) after SHLD
    uint16_t cf_dest = 0x8000;
    uint16_t cf_src = 0x0001;
    uint8_t cf_flag = 0;
    __asm__ __volatile__ (
        "shldw $1, %2, %1\n\t"
        "setc %0"
    ...
    printf("16-bit SHLD CF test: CF=%d\n", cf_flag);
    if (cf_flag != 1) {
    ...
    uint32_t mask_dest = 0x00000001;
    uint32_t mask_src = 0xFFFFFFFF;
    uint32_t mask_expected = (mask_dest << 1) | (mask_src >> 31);
    __asm__ __volatile__ (
        "shldl $33, %1, %0"  // 33 masked to 1 (32-bit: 33 mod 32 = 1)
        : "+r" (mask_dest)
        : "r" (mask_src)
    );
    
    printf("32-bit masked SHLD (<<33 masked to 1): 0x%08X\n", mask_dest);
