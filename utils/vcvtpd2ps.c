...
// 定义指令集宏 (CVTPD2PS)
#ifdef LOONGARCH
#define TEST_INSTR(x) __lasx_xvfcvt_s_d(x)
#else
#define TEST_INSTR(x) _mm256_cvtpd_ps(x)
#endif
...
// 打印float值的十六进制表示
void print_float_hex(float f, char *buffer) {
...
void run_tests() {
    ...
    for (int m = 0; m < num_modes; m++) {
        ...
        for (int i = 0; i < num_cases; i++) {
            ...
            __m128 res = TEST_INSTR(a);
            __lsx_vst(res, res_vec, 0);
            #else
            __m256d a = _mm256_loadu_pd(a_vec);
            __m128 res = TEST_INSTR(a);
            _mm_storeu_ps(res_vec, res);
            #endif
            
            float result = res_vec[0];
            
            // 准备十六进制表示
            char a_hex[20], res_hex[20];
            print_double_hex(tc.a, a_hex);
            print_float_hex(result, res_hex);
            
            // CSV格式输出
            printf("\"%s\",\"%s\",%.17g,%s,%.17g,%s\n",
