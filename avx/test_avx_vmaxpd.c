...
// vmaxpd 指令测试
static void test_vmaxpd() {
    printf("--- Testing vmaxpd ---\n");
    ...
    double expected[4] = {
        1.0,   // max(1.0, -1.0)
        3.0,   // max(-2.0, 3.0)
        -0.0,  // max(0.0, -0.0) - 根据指令行为返回第二个操作数(-0.0)
    ...
    for (int i = 0; i < 4; i++) {
        if (isnan(expected[i])) {
            if (!isnan(dst[i])) {
                printf("Error at element %d: expected NaN, got %f\n", i, dst[i]);
                errors++;
            }
        } else if (expected[i] == 0.0 && dst[i] == 0.0) {
            // 检查符号位
            uint64_t exp_bits = *(uint64_t*)&expected[i];
    ...
    // 检查结果 - 测试2
    double expected2[4] = {
        INFINITY,    // max(INFINITY, 0.0) -> INFINITY
        NAN,         // max(-INFINITY, NAN) -> NAN
        INFINITY,    // max(NAN, INFINITY) -> INFINITY (根据规范，当有一个操作数是NaN时，结果可能是另一个操作数)
        0.0          // max(-0.0, 0.0) -> +0.0
    };
    ...
    if (errors == 0) {
        printf("vmaxpd (256-bit) test passed!\n");
    } else {
        printf("vmaxpd (256-bit) test failed with %d errors\n", errors);
    }
    ...
    if (errors128 == 0) {
        printf("vmaxpd (128-bit) test passed!\n");
    } else {
        printf("vmaxpd (128-bit) test failed with %d errors\n", errors128);
    }
    ...
    if (total_errors == 0) {
        printf("All vmaxpd tests passed!\n");
    } else {
        printf("vmaxpd tests failed with %d total errors\n", total_errors);
    }
