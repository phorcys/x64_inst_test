...
static void test_vunpcklpd_128() {
    printf("=== Testing vunpcklpd (128-bit) ===\n");
...
static void test_vunpcklpd_256() {
    ...
    // 通道0: [a0, b0]
    // 通道1: [a2, b2]
    expected[0] = a[0]; // 通道0低64位
    expected[1] = b[0]; // 通道0低64位
    expected[2] = a[2]; // 通道1低64位
    expected[3] = b[2]; // 通道1低64位
    ...
    for (int i = 0; i < 4; i++) {
        if (isnan(expected[i])) {
            ...
            pass = pass && double_equal(dst[i], expected[i], EPSILON);
        }
    }
