...
void test_vperm2f128() {
    printf("==== Testing VPERM2F128 (25 cases) ====\n");
    ...
    for (int i = 0; i < 25; i++) {
        ...
        // 处理低位字段
        if (low_zero) {
            memset(&expected[i][0], 0, 4*sizeof(float));
        } else {
            const float* src = (low_sel & 0x02) ? src2 : src1;
            int offset = (low_sel & 0x01) ? 4 : 0;
            memcpy(&expected[i][0], &src[offset], 4*sizeof(float));
        }
        
        // 处理高位字段
        if (high_zero) {
            memset(&expected[i][4], 0, 4*sizeof(float));
        } else {
            const float* src = (high_sel & 0x02) ? src2 : src1;
            int offset = (high_sel & 0x01) ? 4 : 0;
            memcpy(&expected[i][4], &src[offset], 4*sizeof(float));
        }
    }
