...
static void test_reg_reg_128() {
    for (int t = 0; t < TEST_CASE_COUNT; t++) {
        ...
        // 内联汇编实现 VFMADDSUB132PD (128位寄存器-寄存器)
...
static void test_reg_mem_128() {
    for (int t = 0; t < TEST_CASE_COUNT; t++) {
        ...
        // 内联汇编实现 VFMADDSUB132PD (128位寄存器-内存)
        __asm__ __volatile__(
            "vfmaddsub132pd %[b], %[c], %[a]"
            : [a] "+x" (va)
            : [b] "m" (*b_ptr), [c] "x" (vc)
    ...
    for (int t = 0; t < special_count; t++) {
        __m128d va = _mm_load_pd(special_cases_128[t].a);
        double* b_ptr = special_cases_128[t].b;
        __m128d vc = _mm_load_pd(special_cases_128[t].c);
...
static void test_reg_reg_256() {
    for (int t = 0; t < TEST_CASE_COUNT; t++) {
        ...
        // 内联汇编实现 VFMADDSUB132PD (256位寄存器-寄存器)
        __asm__ __volatile__(
            "vfmaddsub132pd %[b], %[c], %[a]"
            : [a] "+x" (va)
            : [b] "x" (vb), [c] "x" (vc)
...
static void test_reg_mem_256() {
    for (int t = 0; t < TEST_CASE_COUNT; t++) {
        ...
        // 内联汇编实现 VFMADDSUB132PD (256位寄存器-内存)
        __asm__ __volatile__(
            "vfmaddsub132pd %[b], %[c], %[a]"
            : [a] "+x" (va)
            : [b] "m" (*b_ptr), [c] "x" (vc)
    ...
    for (int t = 0; t < special_count; t++) {
        __m256d va = _mm256_load_pd(special_cases_256[t].a);
        double* b_ptr = special_cases_256[t].b;
        __m256d vc = _mm256_load_pd(special_cases_256[t].c);
...
int main() {
    ...
    printf("All VFMADDSUB132PD tests completed. Results are for verification on physical CPU vs box64.\n");
