...
static void test_reg_reg_128() {
    for (int t = 0; t < TEST_CASE_COUNT; t++) {
        ...
        // 内联汇编实现 VFMADDSUB213PS (128位寄存器-寄存器)
        __asm__ __volatile__(
            "vfmaddsub213ps %[b], %[c], %[a]"
            : [a] "+x" (va)
            : [b] "x" (vb), [c] "x" (vc)
        );
...
static void test_reg_mem_128() {
    for (int t = 0; t < TEST_CASE_COUNT; t++) {
        __m128 va = _mm_load_ps(fma_cases_128_ps[t].a);
        __m128 vb = _mm_load_ps(fma_cases_128_ps[t].b);
        float* c_ptr = fma_cases_128_ps[t].c;
        
        // 内联汇编实现 VFMADDSUB213PS (128位寄存器-内存)
        __asm__ __volatile__(
            "vfmaddsub213ps %[c], %[b], %[a]"
            : [a] "+x" (va)
            : [b] "x" (vb), [c] "m" (*(const __m128*)c_ptr)
        );
...
static void test_reg_reg_256() {
    for (int t = 0; t < TEST_CASE_COUNT; t++) {
        ...
        // 内联汇编实现 VFMADDSUB213PS (256位寄存器-寄存器)
        __asm__ __volatile__(
            "vfmaddsub213ps %[b], %[c], %[a]"
            : [a] "+x" (va)
            : [b] "x" (vb), [c] "x" (vc)
...
static void test_reg_mem_256() {
    for (int t = 0; t < TEST_CASE_COUNT; t++) {
        __m256 va = _mm256_load_ps(fma_cases_256_ps[t].a);
        __m256 vb = _mm256_load_ps(fma_cases_256_ps[t].b);
        float* c_ptr = fma_cases_256_ps[t].c;
        
        // 内联汇编实现 VFMADDSUB213PS (256位寄存器-内存)
        __asm__ __volatile__(
            "vfmaddsub213ps %[c], %[b], %[a]"
            : [a] "+x" (va)
            : [b] "x" (vb), [c] "m" (*(const __m256*)c_ptr)
        );
...
int main() {
    ...
    printf("All VFMADDSUB213PS tests completed. Results are for verification on physical CPU vs box64.\n");
