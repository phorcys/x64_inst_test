...
int main() {
    ...
    // 测试比例因子
    printf("\nTesting scale factor 8\n");
    // 重新设置索引数组以测试比例因子
    indices[0] = 0; // 0 * 8 = 0
    indices[1] = 1; // 1 * 8 = 8 (索引1)
    ...
    printf("Expected: [");
    for (int i = 0; i < 4; i++) {
        printf("0x%016lx%s", mem256_data[indices256[i]], i < 3 ? ", " : "");
    }
    printf("]\n");
    ...
    if (valid) {
        printf("\nAll VPGATHERQQ tests completed successfully.\n");
        return 0;
    } else {
        return 1;
    }
