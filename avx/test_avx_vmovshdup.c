...
void test_vmovshdup() {
    ...
    // 测试内存操作数版本
    {
        ...
        printf("\nTesting vmovshdup (128-bit memory):\n");
        ...
        if(dst[0] == src[1] && dst[1] == src[1] &&
           dst[2] == src[3] && dst[3] == src[3]) {
            printf("  PASS: 128-bit vmovshdup (memory)\n");
        } else {
            printf("  FAIL: 128-bit vmovshdup (memory)\n");
        }
    }
    ...
    // 测试边界条件
    {
        ...
        for(size_t i = 0; i < sizeof(test_cases)/sizeof(test_cases[0]); i++) {
            ...
            // 验证结果
            int pass = 1;
            if(isnan(src[1]) || isnan(src[3])) {
                pass &= isnan(dst[0]) && isnan(dst[1]) &&
                       isnan(dst[2]) && isnan(dst[3]);
            } else {
                pass &= (dst[0] == src[1]) && (dst[1] == src[1]) &&
                       (dst[2] == src[3]) && (dst[3] == src[3]);
            }
