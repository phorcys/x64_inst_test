...
static void test_reg_reg_128() {
    for (int t = 0; t < TEST_CASE_COUNT; t++) {
        ...
        // 内联汇编实现 VFMADDSUB231PS (128位寄存器-寄存器)
...
static void test_reg_mem_128() {
    for (int t = 0; t < TEST_CASE_COUNT; t++) {
        ...
        // 内联汇编实现 VFMADDSUB231PS (128位寄存器-内存)
        __asm__ __volatile__(
            "vfmaddsub231ps %[c], %[b], %[a]"
            : [a] "+x" (va)
            : [b] "m" (*b_ptr), [c] "x" (vc)
...
static void test_reg_reg_256() {
    for (int t = 0; t < TEST_CASE_COUNT; t++) {
        ...
        // 内联汇编实现 VFMADDSUB231PS (256位寄存器-寄存器)
        __asm__ __volatile__(
            "vfmaddsub231ps %[c], %[b], %[a]"
            : [a] "+x" (va)
            : [b] "x" (vb), [c] "x" (vc)
...
static void test_reg_mem_256() {
    for (int t = 0; t < TEST_CASE_COUNT; t++) {
        ...
        // 内联汇编实现 VFMADDSUB231PS (256位寄存器-内存)
        __asm__ __volatile__(
            "vfmaddsub231ps %[c], %[b], %[a]"
            : [a] "+x" (va)
            : [b] "m" (*b_ptr), [c] "x" (vc)
...
int main() {
    ...
    printf("All VFMADDSUB231PS tests completed. Results are for verification on physical CPU vs box64.\n");
